...
class ConvModel(nn.Module):
    ...
    def __init__(self, model_dim: int = 384, kernel_size: int = 15, num_cycles: int = 0):
        super().__init__()
    ...
    def forward(self, image, use_memristor=False):
        ...
        if use_memristor:
            ...
            conv_out = self.conv_1(self.activation_quant_l2_in(out1).transpose(1, 2)).transpose(1, 2)
...
@pytest.mark.linear
def test_conv1d():
    run_training(ConvModel, expected_accuracy=0.8, batch_size=100, num_cycles=1, num_epochs=2)
    # print("Num Cycle = 1")
